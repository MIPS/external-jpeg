...
# IDCT implementation using the MIPS DSP ASE (big endian version)
#
...
loop_rows:
        ...
        lw              $t0, 0+0($a0)           # wsptr[DCTSIZE*0+0/1]  a b
        lw              $s0, 16+0($a0)          # wsptr[DCTSIZE*1+0/1]  A B
        lw              $t2, 0+4($a0)           # wsptr[DCTSIZE*0+2/3]  c d
        lw              $s2, 16+4($a0)          # wsptr[DCTSIZE*1+2/3]  C D
        lw              $t4, 0+8($a0)           # wsptr[DCTSIZE*0+4/5]  e f
        lw              $s4, 16+8($a0)          # wsptr[DCTSIZE*1+4/5]  E F
        lw              $t6, 0+12($a0)          # wsptr[DCTSIZE*0+6/7]  g h
        lw              $s6, 16+12($a0)         # wsptr[DCTSIZE*1+6/7]  G H

        precrq.ph.w     $t1, $s0, $t0           # A a
        ins             $t0, $s0, 16, 16        # B b
...
full_row:
        ...
        precrq.ph.w     $t3, $s2, $t2           # C c
        ins             $t2, $s2, 16, 16        # D d

        precrq.ph.w     $t5, $s4, $t4           # E e
        ins             $t4, $s4, 16, 16        # F f

        precrq.ph.w     $t7, $s6, $t6           # G g
        ins             $t6, $s6, 16, 16        # H h


        lw              $t8, 4($at)             # FIX(1.414213562)

        addq.ph         $s4, $t1, $t5           # tmp10
        subq.ph         $s5, $t1, $t5           # tmp11

        subq.ph         $s6, $t3, $t7           # tmp12 ...
        addq.ph         $s7, $t3, $t7           # tmp13

        mulq_rs.ph      $s6, $s6, $t8           # ... tmp12 ...

        addq.ph         $t1, $s4, $s7           # tmp0
        subq.ph         $t7, $s4, $s7           # tmp3

        shll_s.ph       $s6, $s6, 1             # x2

        subq.ph         $s6, $s6, $s7           # ... tmp12

        addq.ph         $t3, $s5, $s6           # tmp1
        subq.ph         $t5, $s5, $s6           # tmp2

################

        addq.ph         $s5, $t0, $t6           # z11
        subq.ph         $s6, $t0, $t6           # z12

        addq.ph         $s7, $t4, $t2           # z13
        subq.ph         $v0, $t4, $t2           # z10

        addq.ph         $t6, $s5, $s7           # tmp7
        subq.ph         $s5, $s5, $s7           # tmp11 ...
        ...
        addq.ph         $s0, $t1, $t6           # tmp0 + tmp7
        subq.ph         $s7, $t1, $t6           # tmp0 - tmp7
        ...
        shll_s.ph       $s0, $s0, SHIFT         # A a

        subq.ph         $t4, $s6, $t6           # tmp6
        subq.ph         $s4, $s4, $v1           # ... tmp10
        subq.ph         $t2, $s5, $t4           # tmp5

        shll_s.ph       $s7, $s7, SHIFT         # H h

        addq.ph         $t0, $s4, $t2           # tmp4


        addq.ph         $s1, $t3, $t4           # tmp1 + tmp6
        subq.ph         $s6, $t3, $t4           # tmp1 - tmp6

        addq.ph         $s2, $t5, $t2           # tmp2 + tmp5
        subq.ph         $s5, $t5, $t2           # tmp2 - tmp5

        addq.ph         $s4, $t7, $t0           # tmp3 + tmp4
        subq.ph         $s3, $t7, $t0           # tmp3 - tmp4


        shll_s.ph       $s1, $s1, SHIFT         # B b
        shll_s.ph       $s2, $s2, SHIFT         # C c
        shll_s.ph       $s3, $s3, SHIFT         # D d
        shll_s.ph       $s4, $s4, SHIFT         # E e
        shll_s.ph       $s5, $s5, SHIFT         # F f
        shll_s.ph       $s6, $s6, SHIFT         # G g


        precrq.ph.w     $t0, $s0, $s1           # A B
        ins             $s1, $s0, 16, 16        # a b

        precrq.ph.w     $t2, $s2, $s3           # C D
        ins             $s3, $s2, 16, 16        # c d

        precrq.ph.w     $t4, $s4, $s5           # E F
        ins             $s5, $s4, 16, 16        # e f

        precrq.ph.w     $t6, $s6, $s7           # G H
        ins             $s7, $s6, 16, 16        # g h

        precrq.qb.ph    $t0, $t0, $t2           # A B C D
        precrq.qb.ph    $s0, $s1, $s3           # a b c d

        precrq.qb.ph    $t4, $t4, $t6           # E F G H
        precrq.qb.ph    $s4, $s5, $s7           # e f g h
        ...
        sw              $s0, 0($a3)             # outptr[0/1/2/3]       a b c d
        sw              $s4, 4($a3)             # outptr[4/5/6/7]       e f g h
        ...
        sw              $t0, 0($a3)             # outptr[0/1/2/3]       A B C D

        bne             $a0, $t9, loop_rows
        sw              $t4, 4($a3)             # outptr[4/5/6/7]       E F G H
